<?php include ("header.php") ?>
    <section class="yararli-link">
      <div class="container">
        <div class="row">
          <div class="col-12 mb-5">
            <h3 class="h3-heading text-center">Yararlı Linkler</h3>
          </div>
          <div class="row mt-5 mb-5">
            <div class="col link-group">
              <a
                href="https://www.suverimliligi.gov.tr/"
                class="bg-primary text-white py-4 px-3 rounded-1"
              >
                Su Verimliliği
              </a>
              <a
                href="https://www.suen.gov.tr/"
                class="bg-primary text-white py-4 px-3 rounded-1"
              >
                SUEN (Su Enstitüsü)
              </a>
              <a
                href="https://www.tarimorman.gov.tr/SYGM"
                class="bg-primary text-white py-4 px-3 rounded-1"
              >
                Su Yönetimi Genel Müdürlüğü
              </a>
              <a
                href="https://sutema.org/"
                class="bg-primary text-white py-4 px-3 rounded-1"
              >
                SuTEMA
              </a>
              <a
                href="https://www.stormsaver.com/"
                class="bg-primary text-white py-4 px-3 rounded-1"
              >
                Stormsaver
              </a>
              <a
                href="https://watereurope.eu/"
                class="bg-primary text-white py-4 px-3 rounded-1"
              >
                WaterEurope
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
    <hr class="my-5" />
    <section class="iletisim mt-5 mb-5">
      <div class="container">
        <div class="row border10">
          <div class="col-12 d-flex justify-content-center align-items-center">
            <h3 class="h3-heading d-inline-flex h3-heading--iletisim">
              İletişim
            </h3>
          </div>
          <div class="col-12 d-flex justify-content-center align-items-center">
            <div class="iletisim-detay d-flex align-items-center">
              <div class="iletisim-detay--icon">
                <img src="assets/img/mailbox.svg" alt="" srcset="" />
              </div>
              <div class="iletisim-detay--text">
                <p>
                  Su Hasadı hakkında bilgi almak ve iletişime geçmek için mail
                  adresimizi kullanabilirsiniz.
                </p>
                <h3 class="h3-heading">info@suhasadi.com</h3>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <hr />
<?php include ("footer.php") ?>
